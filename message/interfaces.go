...
// 消息的命令别名
//
// 例如 CmdDanmakuWithCode 便是 Danmaku 的命令别名
//
...
type Parser interface {
	// 将单条消息解析成符合 Msg 接口的对象
	// 允许返回空接口
	UnmarshalMessage([]byte) (Msg, error)
}

// 将解析器转为符合 (*Core).Handler 的函数
func ParseFunc(p Parser, fn func(*core.Core, Msg)) func(*core.Core, []byte) {
	return func(c *core.Core, b []byte) {
		msg, err := p.UnmarshalMessage(b)
		if err != nil {
			c.Error(err)
		}
		fn(c, msg)
